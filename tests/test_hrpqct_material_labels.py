...
MODULE_DIR = Path(__file__).resolve().parents[1] / "HRpQCTSegmentation"
sys.path.insert(0, str(MODULE_DIR))

import HRpQCTSegmentation  # noqa: E402
...
def test_material_label_arrays_combine_segmentation_and_compartments():
    ...
    material, counts = HRpQCTSegmentation._material_labels_from_arrays(
        seg,
        trab,
        cort,
        trab_label=126,
...
def test_derive_compartment_arrays_support_any_two_inputs():
    ...
    derived_full = HRpQCTSegmentation._derive_compartment_mask_arrays(trab=trab, cort=cort)
    derived_trab = HRpQCTSegmentation._derive_compartment_mask_arrays(full=full, cort=cort)
    derived_cort = HRpQCTSegmentation._derive_compartment_mask_arrays(full=full, trab=trab)
...
def test_material_labels_can_use_derived_trabecular_region():
    ...
    cort = np.zeros_like(seg)
    cort[0] = True
    masks = HRpQCTSegmentation._derive_compartment_mask_arrays(full=full, cort=cort)
...
def test_compartment_validation_detects_overlap_and_missing_voxels():
    ...
    counts = HRpQCTSegmentation._validate_compartment_mask_arrays(full=full, trab=trab, cort=cort)
...
def test_boolean_mask_operations_and_relabel_nonzero():
    ...
    assert HRpQCTSegmentation._binary_mask_operation_arrays(mask_a, mask_b, "union").tolist() == [
        True,
        True,
        True,
        False,
    ]
    assert HRpQCTSegmentation._binary_mask_operation_arrays(mask_a, mask_b, "intersection").tolist() == [
        False,
        True,
        False,
        False,
    ]
    assert HRpQCTSegmentation._binary_mask_operation_arrays(mask_a, mask_b, "difference").tolist() == [
        False,
        False,
        True,
        False,
    ]
    assert HRpQCTSegmentation._relabel_nonzero_array(mask_a, 127).tolist() == [0, 127, 127, 0]
