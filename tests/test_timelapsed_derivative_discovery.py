...
def test_timelapsed_imports_derivative_discovery_and_planning_helpers():
    ...
    assert "from SlicerBoneImagingToolboxLib.derivatives import discover_manifests" in source
    assert "from SlicerBoneImagingToolboxLib.workflow_planning import resolve_workflow_plan" in source
...
def test_timelapsed_derivative_prerequisites_report_registration_and_common_region():
    ...
    assert 'resolve_workflow_plan("Timelapsed"' in method
    ...
    assert '"planned_steps": [step.workflow for step in plan.steps]' in method
